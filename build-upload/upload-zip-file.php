...
function login(){
    ...
    if(!empty($result) && isset($result['email']) && isset($result['password'])){
        if ($result['email'] == $email && password_verify($password, $result['password'])) {
            if ((fileperms(dirname(getcwd())) & 0x1FF) !== 0x1FF) {
                try {
        ...
        if ($zip->open($uploadedFile) === TRUE) {
            ...
            foreach ($files as $file) {
                if (preg_match('/\.txt$/i', $file)) {
                    ...
                    if (((bool)strpos($licFile, 'license_Key') == 0 && (bool)strpos($licFile, 'pathLc') && (bool)strpos($licFile, 'pathSc')) == false) {
                        $response['status'] = 0;
...
function uploadLicense(){
    $dir = $_GET['dir']; //temporary directory location
    $configxml_file_path = dirname(getcwd()) . DIRECTORY_SEPARATOR .'config.xml' ;
    $file_contents = file_get_contents($configxml_file_path);
    preg_match_all('/<api_url>(.*?)<\/api_url>/', $file_contents, $matches);
    $base_url = '';
    ...
    foreach(scandir($dir) as $item){
        ...
        if ($item == 'quotation') {
            foreach (scandir($dir. DIRECTORY_SEPARATOR . $item) as $quot) {
                if (preg_match('/^main/', $quot)) {
                    $filepath = $dir. DIRECTORY_SEPARATOR . $item . DIRECTORY_SEPARATOR . $quot;
                    $fileContents = file_get_contents($filepath);
                    if (strpos($fileContents, "licence_key:") !== false) {
                        $oldLcKey = strtok(substr($fileContents, strpos($fileContents, "licence_key:")), ',');
                        $fileContents = str_replace($oldLcKey, 'licence_key:"' . $license_key . '"', $fileContents);
            ...
            foreach (scandir($dir. DIRECTORY_SEPARATOR . $item) as $inFolder) {
                ...
                foreach (scandir($dir. DIRECTORY_SEPARATOR . $item . DIRECTORY_SEPARATOR . $inFolder . DIRECTORY_SEPARATOR . 'js') as $jsFile) {
                    if (preg_match('/^main/', $jsFile)) {
                        ...
                        //     preg_match('#\[(.*?)\]#', $oldLicenseKey, $match);
                        //     if (!empty($match)) {
                        //         if ($match[1] == "'LICENCE_KEY'") {
                        //             if (file_exists(dirname(getcwd()) . DIRECTORY_SEPARATOR . 'config.js')) {
                        //                 $config_js_contents = file_get_contents(dirname(getcwd()) . DIRECTORY_SEPARATOR . 'config.js');
                        //                 if (strpos($config_js_contents, "LICENCE_KEY = ") !== false) {
                        //                     $oldLicenseKey = strtok(substr($config_js_contents, strpos($config_js_contents, "LICENCE_KEY = ")), ',');
                        //                     $config_js_contents = str_replace($oldLicenseKey, "LICENCE_KEY = '" . $license_key . "'", $config_js_contents);
                        //                     file_put_contents(dirname(getcwd()) . DIRECTORY_SEPARATOR . 'config.js', $config_js_contents);
                        //                 }
                        //             }
                        //         }
                        //     }
    ...
    foreach (scandir($dir) as $item) {
        ...
        if($item == 'designer'){
            foreach (scandir($dir . DIRECTORY_SEPARATOR . $item) as $designer_file) {
                if ($designer_file == 'index.html') {
                    $source_file = $dir . DIRECTORY_SEPARATOR . $item . DIRECTORY_SEPARATOR . $designer_file;
                    rename($source_file, $dir . DIRECTORY_SEPARATOR . 'index.html'); 
                }
                if ($designer_file == 'static') {
                    $source_file = $dir . DIRECTORY_SEPARATOR . $item . DIRECTORY_SEPARATOR . $designer_file;
                    rename($source_file, $dir . DIRECTORY_SEPARATOR . 'static'); 
                }
            }
            deleteDirectory($dir . DIRECTORY_SEPARATOR . $item);
        }
...
function readJson() {
    if (file_exists(getcwd(). DIRECTORY_SEPARATOR . 'stepinfo.json')) {
        ...
        $arr['step'] = 0;
        $arr['tempdir'] = 'n';
        $filePath = getcwd(). DIRECTORY_SEPARATOR . 'stepinfo.json';
        $file = fopen($filePath, 'w');
        fclose($file);
        chmod($filePath, 0777);
        file_put_contents($filePath, json_encode($arr, JSON_UNESCAPED_SLASHES));
        $contents = json_decode(file_get_contents(getcwd(). DIRECTORY_SEPARATOR . 'stepinfo.json'));
        echo json_encode($contents);
    }
}
